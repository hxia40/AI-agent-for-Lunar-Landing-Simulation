...
def make_train_sets_old(num_train_set=100,num_sequences=10, random_seed=1):
    # generate sequences like "454567", instead of "DEDEFG", as the former is way easier to code.
    np.random.seed(random_seed)
    all_sets = []
    for each_train_set in range(num_train_set):
...
if __name__ == '__main__':
    # all_sets = make_train_sets(num_train_set=100,
    #                            num_sequences=10,
    #                            random_seed=1,
    #                            length_limit=9999)  # somehow then num_sequence larger than 100, say 1000, the value estimate will go crazy. using a smaller alhpa helps.
    all_sets = make_train_sets_old(num_train_set=100,
                               num_sequences=10,
                               random_seed=1,)

    lam_list = [0.0, 0.1, 0.3, 0.5, 0.7, 0.9, 1.0]
    error_collection = []
    for each_lam in lam_list:
        print("running lambda = ", each_lam)
        train_set_enum = 0
        for each_train_set in all_sets:
            ...
            train_set_enum += 1
            print("this is the ", train_set_enum, "th trainset")
        ...
        error_collection.append(point_error)

    '''plotting the error value for each lambda as Figure 3'''

    plt.grid()

    plt.plot(lam_list, error_collection)

    plt.xlabel('λ')
    plt.ylabel("ERROR")
    plt.savefig('Figure3_alpha_0.01_delta_0.001_rdmseed_1.png')
