...
from HX_QLearner import QLearningTable, QLearningTableNC, QLearningTableDN
...
def Q_DN(random_seed=1):
    ...
    range_end = 100000
    env_DN.seed(random_seed)
    for episode in range(range_end):
        # alpha = (1 - math.log(episode+1, 10) / math.log(range_end, 10))/10
        alpha = 0.01
        if (episode + 1) % (range_end / 10) == 0:
            print("episode = ", episode + 1, "learnng rate = ", alpha, "reward = ", np.mean(reward_list_jr), "state = ",
                  np.mean(state_list_jr))
            episode_list.append(episode + 1)
            time_list.append(np.mean(time_list_jr))
            time_list_jr = []
            reward_list.append(np.mean(reward_list_jr))
            reward_list_jr = []
            state_list.append(np.mean(state_list_jr))
            state_list_jr = []
        # initial observation
        observation = env_DN.reset()
        start_time = time.time()
        while True:
            # # fresh env
            # env_NC.render()
            ...
            # QL choose action based on observation
            action = QL_DN.choose_action(observation)
            # print env_NC.step(action)

            # QL take action and get next observation and reward
            observation_, reward, done, info = env_DN.step(action)
            # print action
            # print env_NC.step(action)

            # QL learn from this transition
            QL_DN.learn(observation, action, reward, observation, alpha)
            ...
            if done:
                time_list_jr.append(time.time()-start_time)
                reward_list_jr.append(reward)
                state_list_jr.append(observation)
                break
...
if __name__ == "__main__":
    ...
    QL_DN = QLearningTableDN(actions=list(range(2)),
                             learning_rate=0.01,
                             reward_decay=1,
                             e_greedy=1,
                             )
