...
CE-Q learner finds correlated-equilibria using LP. I implemented utilitarian CE-Q (uCE-Q), in which CE maximizes the
sum of both agent's rewards. For that, I first computed probabilistic action space (PAS) that maximizes each agent's
rewards. Next, using PAS and agent's Q tables, I found the most optimal action to take at given state for each agent.
Next, using these actions and PAS table, I found s' and updated Q tables.
...
class CE_Q_table:
    ...
    def __init__(self,
        ...
        self.S = SoccerEnv().S
        self.A = SoccerEnv().A
        self.q_table = pd.DataFrame(0,index=pd.MultiIndex.from_tuples(self.S),
    ...
    def choose_action(self, s, epsilon):
        ...
        if np.random.random() > epsilon:
            ...
            state_action_A = self.q_table.loc[s, :]
            state_action_B = state_action_A * (-1)
    ...
    def learn(self, s, a, r, s_, a_, alpha, gamma):
        ...
        if 0 in r:
            q_target = r[0] + gamma * self.q_table.loc[s_, a_]  # next state is not terminal

            # print("friend_Q.py line 61, self.q_table_A.loc[s_, a_[0]] ", self.q_table_A.loc[s_, a_])
        else:
            q_target = r[0]  # next state is terminal
    ...
    def return_Q_table(self):
        # print("ce_Q, line 177, type(self.q_table)", type(self.q_table))
        return self.q_table
