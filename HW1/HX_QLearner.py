...
class QLearningTableDN:
    def __init__(self, actions, learning_rate=0.01, reward_decay=0.9, e_greedy=0.9, verbose=1):
        self.actions = actions  # a list
        self.lr = learning_rate
        self.gamma = reward_decay
        self.epsilon = e_greedy
    ...
    def learn(self, s, a, r, s_, alpha):
        ...
        # if s_ != self.total_length:
        # '''this problem does not have total length, remove'''
        #     q_target = r + self.gamma * self.q_table.loc[s_, :].max()  # next state is not terminal
        # else:
        #     q_target = r  # next state is terminal
        q_target = r + self.gamma * self.q_table.loc[s_, :].max()  # next state is not terminal
        # self.q_table.loc[s, a] += self.lr * (q_target - q_predict)  # update , Morvan's original
        self.q_table.loc[s, a] += alpha * (q_target - q_predict)  # update , HX self defined alpha
